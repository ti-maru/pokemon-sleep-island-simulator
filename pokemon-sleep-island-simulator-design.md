...
| ガイド | 簡易ヘルプ、更新履歴、GitHubリポジトリへのリンク |
...
- タイムゾーン名には `UTC+09:00 — Asia/Tokyo` の形式で現在時点のUTCオフセットを併記し、夏時間で変化する可能性を案内
- 検証モード
- PWAインストール案内の再表示
- バックアップ・復元
- 保存容量表示・履歴整理
...
- 各選択肢には現在時点のUTCオフセットを `UTC±HH:MM` 形式で併記する。30分・45分単位の地域および夏時間を考慮する
- 選択肢はUTCオフセットの昇順とし、同一オフセット内ではIANAタイムゾーンID順に並べる
...
- 選択肢と個体カードのポケモン名には `No.001 フシギダネ` のように図鑑番号を付記する。番号は3桁未満をゼロ埋めし、1000以上は桁を省略しない
- 経験値タイプを600／900／1080／1320から手動上書き可能
- 上書き中はポケモンマスター更新の影響を受けない
- 新規ポケモンが未登録でも、手動タイプ選択で計算可能
- ポケモン選択と経験値タイプ上書きチェックは視覚的に分離し、上書きチェックを経験値タイプ欄へ関連付けて配置する
...
- ナビゲーション、タブ、ボタン、無効入力、空状態、表の強調行、グラフを含む全画面でテーマ別の意味色を使用し、ライト用の固定背景色をダークテーマへ残さない
...
| 1.3 | 2026-07-20 | ライト・ダーク全画面の意味色とコントラスト、図鑑番号付き名称・番号順、計算画面のポケモン／経験値タイプ配置、タイムゾーンのUTCオフセット表示・並び順、GitHubリポジトリリンク要件を追加 |
