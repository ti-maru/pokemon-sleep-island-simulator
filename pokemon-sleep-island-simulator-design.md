...
| 項目 | 役割 |
|---|---|
| 計算 | EXP計算、レベル計算、シナリオ比較、グラフ、目標試算 |
...
- 管理名・ポケモン名検索
- 預け入れ中、目標未達、計画あり等の絞り込み
- 更新日、レベル、目標日、作成日で並び替え
...
- 性格名を選択し、EXP補正を自動判定
- 「EXP上昇・無補正・EXP下降」を直接選択

前回使用した方式を保存する。倍率はマスターデータから取得し、UIコードへ直書きしない。
...
| 目標 | 目標レベル・目標日、個体保存＋一時変更 |
| 計画 | 複数回、3戦略自動生成、手動編集可能 |
